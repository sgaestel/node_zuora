...
function convert (method, object, objectType) {
  ...
  var ns1ForcedPrefix = ['SubscribeOptions'];
  ...
  var toXml = function(obj, depth, forcedPrefix) {
    return _.join(_.map(obj, function(value, key) {
      ...
      if (_.isArray(value)) {
        ...
        // Overwrite prefix if needs to be forced
        if (forcedPrefix) {
            prefix = forcedPrefix;
        }
        ...
        if (_.isObject(value)) {
          prefix = prefix || ns1;
          val = toXml(value, ++depth, _.includes(ns1ForcedPrefix, key) || forcedPrefix ? ns1 : null);
